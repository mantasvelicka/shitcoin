...
uint256 hashGenesisBlock("0x4e24d487f61a6e655b144bab0f96e8a0619ca6e8ee45835975e647c49808f2ab");
...
const string strMessageMagic = "Shitcoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Shitcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Shitcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Shitcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Shitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Shitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfed0120f94bf050b70bc43b646a46b2a7512bebb8377fd3885dbaf237b2b7771");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Best price for shitcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e0e3d019e7a45b3b2f9fb51fa8cba38cee3c7c8d1a01469da5392bbf93c670e76e3cd653c0d9b36bcd5ab54f79684c8c97a80884bc3907b0cc798c7fff4b2430") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517095594;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085186598;

        if (fTestNet)
        {
            block.nTime    = 1517095575;
            block.nNonce   = 385303458;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb120f68b499861a51f68bb6d2cf84af291f68f7ada0d81c81de8ebcf5987e7eb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Shitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ShitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ShitcoinMiner(CWallet *pwallet)
{
    printf("ShitcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ShitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ShitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ShitcoinMiner, pwallet));
}
